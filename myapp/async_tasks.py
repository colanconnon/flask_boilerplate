...
from . import celery
from .utils.url_for import url_for
...
def run_flask_request(environ):
    from .background_app import background_app as app

    if '_wsgi.input' in environ:
        environ['wsgi.input'] = str(environ['_wsgi.input'])
...
def async(f):
    ...
    def wrapped(*args, **kwargs):
        ...
        if 'wsgi.input' in request.environ:
            environ['_wsgi.input'] = str(request.get_data())
